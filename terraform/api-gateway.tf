...
# 📂 Hardcoded Base Route: edgar-data
...
# ✅ Define API Method with Path Parameters
...
resource "aws_lambda_permission" "invoke_authorizer" {
  ...
  action        = "lambda:InvokeFunction"
  function_name = module.authorizer.lambda_function_name  # must match the authorizer’s function
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.edgar_api.execution_arn}/*"
}